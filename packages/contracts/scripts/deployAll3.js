...
/**
 * Complete Deployment Script v2 - Prism Finance with PYUSD
 *
 *
 * Deployment order:
 * 1. PYUSD Reference (Pre-deployed by PayPal)
 * 2. CreditScore
 * 3. LendingPool
 * 4. LoanManager
...
async function main() {
  ...
  console.log("PRISM FINANCE - PYUSD DEPLOYMENT");
  console.log("=".repeat(80));
  console.log("\n");
  ...
  // ============================================
  // 1. Use PYUSD (Pre-deployed)
  // ============================================
  console.log("1/10 Using PayPal USD (PYUSD)...");

  // PYUSD addresses by chain ID
  const PYUSD_ADDRESSES = {
    "11155111": "0xCaC524BcA292aaade2DF8A05cC58F0a65B1B3bB9", // Sepolia
    "1": "0x6c3ea9036406852006290770bedfcaba0e23a0e8" // Mainnet
  };

  const pyusdAddress = PYUSD_ADDRESSES[chainId];
  if (!pyusdAddress) {
    throw new Error(`PYUSD not available on chain ID ${chainId}`);
  }

  deployedContracts.contracts.pyusd = pyusdAddress;
  console.log("✓ Using PYUSD at:", pyusdAddress);
  console.log("  Network: Sepolia Testnet");
  console.log("  Get testnet PYUSD: https://cloud.google.com/application/web3/faucet");
  console.log("\n");
  ...
  const lendingPool = await LendingPool.deploy(pyusdAddress);
  await lendingPool.waitForDeployment();
  const lendingPoolAddress = await lendingPool.getAddress();
  deployedContracts.contracts.lendingPool = lendingPoolAddress;
  console.log("✓ LendingPool deployed to:", lendingPoolAddress);
  console.log("  Stablecoin:", pyusdAddress);
  console.log("\n");
  ...
  const insurancePool = await InsurancePool.deploy(pyusdAddress);
  ...
  const frontendEnvUpdates = {
    ...
    'VITE_PYUSD_ADDRESS': pyusdAddress,
  ...
  console.log("-".repeat(80));
  console.log("PYUSD (PayPal USD):   ", deployedContracts.contracts.pyusd);
  console.log("\n");
